...
const translateSystemNodeText = (text: string, lang: Lang) => {
  if (Object.values(TRANSLATIONS).some(tr => tr.defaultNode === text)) {
    return TRANSLATIONS[lang].defaultNode;
  }

  if (Object.values(TRANSLATIONS).some(tr => tr.magnetNode === text)) {
    return TRANSLATIONS[lang].magnetNode;
  }
  ...
  const toggleLang = () => {
      setLang(prev => {
          ...
          setNodes(currentNodes => currentNodes.map(n => {
              return { ...n, text: translateSystemNodeText(n.text, newLang) };
          }));
          return newLang;
      });
  };

  // 璇█鍒囨崲鍚庢寔涔呭寲鍒?localStorage锛屼繚璇佸埛鏂板悗浠嶇劧淇濇寔鏈€杩戜竴娆￠€夋嫨
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setNodes(prev => prev.map(n => ({
            ...n,
            text: translateSystemNodeText(n.text, lang),
        })));
    }
  }, []);
            ...
            <div className="relative shrink-0">
                 ...
                 {isMenuOpen && (
                     <div className="absolute left-full top-1/2 -translate-y-1/2 ml-4 flex items-center gap-1 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl shadow-2xl border border-slate-200 dark:border-slate-700 rounded-2xl p-2 animate-in fade-in slide-in-from-left-2 z-50 menu-popover whitespace-nowrap">
                         ...
                         <button className="p-3 rounded-xl text-xs font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-teal-600 transition-all flex flex-col items-center justify-center gap-0.5" onClick={toggleLang} title={t.toolbar.lang} aria-label={t.toolbar.lang}><span>{lang === 'zh' ? 'ZH' : 'EN'}</span></button>
                     </div>
                 )}
             </div>
         </div>
                                    ...
                                    snapshots.map((snap, i) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700 hover:border-teal-500 transition-all group">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] text-slate-400">{new Date(snap.timestamp).toLocaleDateString()} 鈥?{snap.nodes.length} nodes</span>
                                            </div>
                                            <button 
